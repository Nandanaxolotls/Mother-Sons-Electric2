...
public class P3RED2NGSnapPoint : MonoBehaviour
{
    ...
    private void SnapObject()
    {
        ...
        // Deactivate previous 3 when activating every 4th (or 7th, 10th, etc.)
        if (currentIndex > 0 && currentIndex % 3 == 0)
        {
            ...
            for (int i = start; i < currentIndex; i++)
            {
                if (i >= 0 && i < objectsToActivate.Count && objectsToActivate[i] != null)
                {
                    ...
                    Debug.Log($"Deactivated object {i + 1}: {objectsToActivate[i].name}");
                }
            }
        }
