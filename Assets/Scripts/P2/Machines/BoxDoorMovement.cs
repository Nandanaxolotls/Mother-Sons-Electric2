...
public class BoxDoorMovement : MonoBehaviour
{
    [Header("References")]
    ...
    {
        if(Tooltip11 != null) { Tooltip11.SetActive(false); }
        ...
        if (!isInDesiredPosition)
        {
            if (rotationDirection == RotationDirection.Clockwise)
                targetRotation = new Vector3(-180f, YValue, ZValue);
            else
                targetRotation = new Vector3(-180f, -YValue, -ZValue);
        }
        else
        {
            if (Tooltip12 != null) { Tooltip12.SetActive(false); }
